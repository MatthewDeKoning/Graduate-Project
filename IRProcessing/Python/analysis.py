...
accountants = []
device_list = ['/dev/video0', '/dev/video1', '/dev/video2', '/dev/video3']
'''
accountant class
...
def process_image(image_values, img_time, q):
...
def analyze(q, num):
   ...
   while 1:
      image_values = capture_data.get_data(device_list[num])
      img_time = time.clock()
      process_image(image_values, img_time, q)
      time.sleep(0)
