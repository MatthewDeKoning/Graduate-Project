...
#test object class
class test_obj:
   def __init__(self, dist, vel, wid):
...
def test_func(mode, speed, distance, width):
   global length_1, length_2, length_3, length_4
   global angle_1, angle_2, angle_3, angle_4
   global FPS_NUM
   if mode == 0:
     ...
     angle_2 = 140*(2.0*math.pi)/360.0
     angle_3 = 140*(2.0*math.pi)/360.0
   ...
   w_err_max = 0
   v_err_max = 0
   my_obj = test_obj(distance, speed, width)
   return (mean(my_obj.delta_14), mean(my_obj.delta_23))
'''
            #print 'Global Lists'
